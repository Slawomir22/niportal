      ...
      switchMap(({ request }) => {
        return authService.registerUser(request).pipe(
          ...
          catchError((errorsResponse: HttpErrorResponse) => {
            return of(
              authUserActions.register_user_failure({
    ...
    return actions$.pipe(
      ...
      switchMap(({ request }) => {
        return authService.loginUser(request).pipe(
          ...
          catchError((errorsResponse: HttpErrorResponse) => {
            return of(
              authUserActions.login_user_failure({
                errors: errorsResponse.error.errors,
              })
            );
          })
    ...
    return actions$.pipe(
      ...
      switchMap(({ user }) => {
        return authService.updateUser(user).pipe(
          ...
          catchError((errorsResponse: HttpErrorResponse) => {
            return of(
              authUserActions.update_user_failure({
                errors: errorsResponse.error.errors,
              })
            );
          })
...
export const logoutEffect = createEffect(
  (
    ...
    return actions$.pipe(
      ...
      tap(() => {
        persistenceService?.set("accessToken", ""), router.navigate(["/"]);
      })
    );
  },
  { functional: true, dispatch: false }
